 ...
 * @description Task repository interface
 * @created     2026-04-24
 * ──────────────────────────────────────────────────────────────
 */
...
export interface ITaskRepository {
  getTasks(userId: string, role: string): Promise<Task[]>;
  createTask(taskData: Partial<Task>): Promise<Task>;
  getTaskById(id: string): Promise<Task>;
  submitBid(taskId: string, pesuruhId: string, offerPrice: number): Promise<boolean>;
  getBidsByTask(taskId: string): Promise<any[]>;
  initiateChat(bidId: string, taskId: string): Promise<string>;
}